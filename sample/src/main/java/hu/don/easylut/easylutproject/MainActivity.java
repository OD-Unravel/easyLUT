...
import team.unravel.easylutapp.R;
...
public class MainActivity extends AppCompatActivity implements FilterAdapter.OnFilterSelected {
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    if (item.getItemId() == R.id.action_identity) {
      ...
      return true;
    } else if (item.getItemId() == R.id.action_full_res) {
      item.setChecked(!item.isChecked());
    ...
    new AsyncTask<Bitmap, Bitmap, Bitmap[]>() {
      ...
      protected Bitmap[] doInBackground(Bitmap... bitmaps) {
        ...
        if (bitmap1 != null && !fullRes) {
          ...
          if (measuredWidth != 0 && measuredHeight != 0 && (bitmap1.getHeight() >= measuredHeight
              || bitmap1.getWidth() >= measuredWidth)) {
      ...
      protected void onPostExecute(Bitmap[] bitmap) {
        ...
        if (filterBitmap == null) {
          ...
          Log.d(TAG, String.format("loaded %dx%d bitmap in %.2fms", filterBitmap.getWidth(), filterBitmap.getHeight(),
    ...
    new AsyncTask<Void, Void, Bitmap>() {
      ...
      protected void onPostExecute(Bitmap bitmap) {
        ...
        if (bitmap == null) {
          ...
          Log.d(TAG, String.format("processed %dx%d bitmap in %.2fms", bitmap.getWidth(), bitmap.getHeight(),
              (System.nanoTime() - start) / 1e6f));
        }
      }
    }.execute();
